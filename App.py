...
_COOKIES_FILE_RAW: str = os.environ.get("YTDL_COOKIES", "/root/ytdl/cookies.txt")
COOKIES_FILE: str | None = _COOKIES_FILE_RAW if os.path.exists(_COOKIES_FILE_RAW) else None
if COOKIES_FILE:
    print(f"[STARTUP] Cookies file found: {COOKIES_FILE}")
else:
    print(f"[STARTUP] WARNING: Cookies file not found at {_COOKIES_FILE_RAW!r} — continuing without it.")
...
def search_videos():
    ...
    if error_holder:
        ...
        # Surface a readable message; hide internal stack details
        if "unavailable" in err_msg.lower() or "private" in err_msg.lower():
            friendly = "Video unavailable or private."
        elif "sign in" in err_msg.lower() or "login" in err_msg.lower():
            friendly = "YouTube requires sign-in for this content. Add a cookies.txt file."
        elif "blocked" in err_msg.lower() or "copyright" in err_msg.lower():
            friendly = "Content is blocked or region-restricted."
        else:
            friendly = f"Search failed: {err_msg[:200]}"
